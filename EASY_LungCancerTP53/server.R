...
ProjectName <- "Lung Cancer TP53 Mutation in CCLE"
...
server <- function(input, output, session) {
    ...
    output$upregpathtitle <- renderText({
        
        FC <- input$pathFC
        paste("(Up-regulated pathway (> ",FC," logFC)",sep = "")
        
    })
    
    output$dnregpathtitle <- renderText({
        
        FC <- input$pathFC
        paste("Down-regulated pathway (> -",FC," logFC)",sep = "")
